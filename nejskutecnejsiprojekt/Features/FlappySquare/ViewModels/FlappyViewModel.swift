...
final class FlappyViewModel: ObservableObject {
    ...
    init(
        ...
        let persistence = persistenceService ?? PersistenceService.shared
        let haptics = hapticsService ?? HapticsService.shared
        
        self.persistenceService = persistence
        self.hapticsService = haptics
        self.config = .config(for: persistence.flappyDifficulty)
    ...
    func setupGame(size: CGSize) {
        let isFirstSetup = gameSize == .zero
        gameSize = size
        
        // Only reset if this is the first setup or game is not active
        if isFirstSetup || gameState == .ready {
            resetGame()
        }
    }
    ...
    func handleTap() {
        ...
        case .ready:
            startGame()
            jump() // Also jump on first tap
        case .playing:
            jump()
        case .gameOver:
            resetGame()
            startGame()
            jump() // Also jump when restarting
        case .paused:
            resumeGame()
        }
    }
    ...
    private func startGame() {
        gameState = .playing
        // Delay first obstacle spawn to give player time
        lastObstacleTime = Date()
        startGameLoop()
    }
